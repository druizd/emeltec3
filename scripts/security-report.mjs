      ...
      if (obj.finding) {
        ...
        findings.push({
          ...
          reference: osv.id
            ...
            : (osv.aliases?.[0] ? `https://nvd.nist.gov/vuln/detail/${osv.aliases[0]}` : ''),
...
function parseSecrets(filePath) {
  return readLines(filePath).map((line, i) => {
    ...
    return {
      ...
      fixAvailable: 'Manual',
      recommendation: 'Move to environment variable or secret manager. Rotate if already committed.',
    }
  })
}
...
function parseTextFindings(filePath, defaultCategory) {
  return readLines(filePath).map(line => {
    ...
    const recMap = {
      ...
      ENV_VALUE:      'Verify this variable is not committed to git. Store secrets in a vault or CI/CD secret store.',
    }
...
// ── Sheet 1: Resumen ─────────────────────────────────────────────────
{
  ...
  for (const f of allFindings) {
    if (!byService[f.service]) byService[f.service] = { CRITICAL:0,HIGH:0,MEDIUM:0,LOW:0,INFO:0 }
...
// ── Sheet 2: Hallazgos ───────────────────────────────────────────────
{
  ...
  allFindings.forEach((f, idx) => {
    ...
    ;['service','category','title','description','location','package',
      'reference','fixAvailable','recommendation'].forEach(key => {
