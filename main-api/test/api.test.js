...
process.env.INTERNAL_API_KEY =
  process.env.INTERNAL_API_KEY || 'test_internal_key_only_for_unit_tests';
...
async function withTestGrpcServer(dbMock, run) {
  ...
  const client = new proto.MainApi(`127.0.0.1:${port}`, grpc.credentials.createInsecure());
...
test('siteTelemetryService expone nivel_freatico cuando la variable Nivel usa la calculadora del pozo', () => {
  clearSrcModules();
  const { buildSiteDashboardData, mapHistoricalDashboardRow } = require(
    path.join(srcRoot, 'services', 'siteTelemetryService.js'),
  );
...
test('siteTelemetryService reconoce uint32 como totalizador en historicos aunque el rol sea generico', () => {
  clearSrcModules();
  const { mapHistoricalDashboardRow } = require(
    path.join(srcRoot, 'services', 'siteTelemetryService.js'),
  );
...
test('GET /api/data/preset soporta 7d y usa el ultimo registro si base_date no viene', async () => {
  ...
  await withTestServer(dbMock, async (baseUrl, calls) => {
    const response = await fetch(`${baseUrl}/api/data/preset?serial_id=PLC-01&preset=7d&keys=REG4`);
    const body = await response.json();
...
test('GET /api/data/preset valida preset y base_date invalida', async () => {
  ...
  await withTestServer(dbMock, async (baseUrl) => {
    const invalidPreset = await fetch(`${baseUrl}/api/data/preset?serial_id=PLC-01&preset=90d`);
