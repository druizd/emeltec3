...
async function run() {
  ...
  try {
    ...
    for (const [serial, data] of liveData) {
      await client.query(
        `INSERT INTO equipo (time, id_serial, data) VALUES (NOW(), $1, $2)`,
        [serial, data]
      );
    }
    ...
    const eventos = [
      [3, 'E200', 'SE201', 'S200', 'OXY1', 17.8, 'Oxígeno bajo en mina: 17.8% (umbral: 19%)',          'critica', true,  true,  new Date(Date.now() - 6 * 3600000)],
      [4, 'E200', 'SE201', 'S200', 'CO1',  35,   'CO peligroso en mina: 35 ppm (umbral: 30)',           'critica', true,  true,  new Date(Date.now() - 4 * 3600000)],
      [1, 'E100', 'SE101', 'S100', 'REG1', 3150, 'Temperatura alta planta: 3150 (umbral: 3000)',        'alta',    true,  false, new Date(Date.now() - 2 * 3600000)],
      [7, 'E300', 'SE301', 'S301', 'PH1',  5.8,  'pH fuera de rango: 5.8 (rango esperado 6.5–8.5)',    'media',   false, false, new Date(Date.now() - 45 * 60000)],
      [6, 'E300', 'SE301', 'S300', 'OXY2', 4.9,  'O₂ disuelto bajo: 4.9 mg/L (umbral mínimo: 5.5)',   'alta',    false, false, new Date(Date.now() - 20 * 60000)],
      [2, 'E100', 'SE101', 'S100', 'REG4', 195,  'Voltaje bajo en red: 195V (umbral mínimo: 200V)',    'critica', false, false, new Date(Date.now() - 5 * 60000)],
    ];
    for (const [alerta_id, empresa_id, sub_empresa_id, sitio_id, variable_key, valor, mensaje, severidad, notificado, resuelta, triggered_at] of eventos) {
      await client.query(
        ...
        [alerta_id, empresa_id, sub_empresa_id, sitio_id, variable_key, valor, mensaje, severidad, notificado, resuelta, triggered_at]
      );
    }
    ...
    rows.forEach(r => console.log(`  ${r.id_serial.padEnd(15)} ${r.registros} registros  último: ${new Date(r.ultimo).toLocaleString('es-CL')}`));
