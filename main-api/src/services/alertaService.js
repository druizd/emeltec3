...
function evalCondicion(condicion, valorNum, umbralBajo, umbralAlto) {
  switch (condicion) {
    case 'mayor_que':   return valorNum > umbralBajo;
    case 'menor_que':   return valorNum < umbralBajo;
    case 'igual_a':     return valorNum === umbralBajo;
    case 'fuera_rango': return valorNum < umbralBajo || valorNum > umbralAlto;
    default:            return false;
  }
}

function diaActual() {
  const localDate = new Date(new Date().toLocaleString('en-US', { timeZone: process.env.ALERT_TIMEZONE || 'America/Santiago' }));
...
function formatCondicion(alerta) {
  switch (alerta.condicion) {
    case 'mayor_que':   return `debe ser mayor que ${alerta.umbral_bajo}`;
    case 'menor_que':   return `debe ser menor que ${alerta.umbral_bajo}`;
    case 'igual_a':     return `debe ser igual a ${alerta.umbral_bajo}`;
    case 'fuera_rango': return `debe estar fuera del rango ${alerta.umbral_bajo} - ${alerta.umbral_alto}`;
    case 'sin_datos':   return `sin datos durante ${alerta.cooldown_minutos} minutos`;
    default:            return alerta.condicion;
  }
}
...
async function evaluarAlerta(client, alerta) {
  ...
  if (!isNaN(valorNum) && evalCondicion(alerta.condicion, valorNum, alerta.umbral_bajo, alerta.umbral_alto)) {
...
async function insertarEvento(client, alerta, valorNum, valorTexto) {
  ...
  const { rows } = await client.query(
    `INSERT INTO alertas_eventos
     ...
     RETURNING id`,
    [alerta.id, alerta.empresa_id, alerta.sub_empresa_id ?? null,
     alerta.sitio_id, alerta.variable_key,
     valorNum, valorTexto, mensaje, alerta.severidad]
  );
...
async function notificarUsuarios(alerta, eventoId, mensaje) {
  ...
  await pool.query(
    'UPDATE alertas_eventos SET notificado = TRUE WHERE id = $1',
    [eventoId]
  );
}
