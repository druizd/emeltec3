    ...
    <div class="space-y-3">
      ...
      <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
        @for (stat of stats(); track stat.label) {
          <div class="rounded-xl border bg-white px-4 py-3 shadow-sm" [class]="stat.borderClass">
            <p class="text-[10px] font-black uppercase tracking-widest" [class]="stat.labelClass">{{ stat.label }}</p>
      ...
      <div class="space-y-2">
        @for (alerta of alertasFiltradas(); track alerta.id) {
          <article class="rounded-2xl border bg-white shadow-sm" [class]="tarjetaBorde(alerta.severidad, alerta.estado)">
            <div class="p-4">
              <div class="flex items-start gap-3">

                <!-- Icono severidad -->
                <span [class]="severidadIconClass(alerta.severidad)" class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl">
                  <span class="material-symbols-outlined text-[20px]">{{ severidadIcon(alerta.severidad) }}</span>
                </span>
                  ...
                  <div class="flex flex-wrap items-start justify-between gap-2">
                    <div class="min-w-0">
                      <div class="flex flex-wrap items-center gap-2">
                        <span class="font-mono text-[11px] font-bold text-slate-400">{{ alerta.codigo }}</span>
                        <span [class]="severidadBadgeClass(alerta.severidad)" class="rounded-full px-2 py-0.5 text-[10px] font-black uppercase tracking-wide">
                    ...
                    <span [class]="estadoBadgeClass(alerta.estado)" class="inline-flex shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-bold">
                      <span [class]="estadoDotClass(alerta.estado)" class="h-1.5 w-1.5 rounded-full"></span>
...
export class AlertasBandejaComponent {
  ...
  alertas: AlertaActiva[] = [
    { id: '1', codigo: 'ALT-0044', variable: 'Tablero eléctrico', mensaje: 'Temperatura superior a 65°C detectada en tablero de control.', severidad: 'critica', estado: 'asignada', fechaInicio: '06/05/2026 10:03', tecnicoAsignado: 'L. Pérez', incidenciaVinculada: 'INC-0018', tiempoTranscurrido: 'Hace 2 h' },
    { id: '2', codigo: 'ALT-0043', variable: 'Caudal mínimo', mensaje: 'Caudal 1.8 L/s — por debajo del umbral de 2.0 L/s.', severidad: 'advertencia', estado: 'reconocida', fechaInicio: '06/05/2026 07:45', tecnicoAsignado: null, incidenciaVinculada: null, tiempoTranscurrido: 'Hace 4 h' },
    { id: '3', codigo: 'ALT-0042', variable: 'Batería de respaldo', mensaje: 'UPS con nivel de batería al 18%. Revisar conexión a red.', severidad: 'advertencia', estado: 'activa', fechaInicio: '06/05/2026 06:00', tecnicoAsignado: null, incidenciaVinculada: null, tiempoTranscurrido: 'Hace 6 h' },
    { id: '4', codigo: 'ALT-0041', variable: 'Sensor de nivel', mensaje: 'Sin lectura por más de 60 minutos. Posible desconexión.', severidad: 'critica', estado: 'resuelta', fechaInicio: '28/04/2026 09:00', tecnicoAsignado: 'L. Pérez', incidenciaVinculada: 'INC-0016', tiempoTranscurrido: 'Hace 8 días' },
  ];
  ...
  readonly stats = computed(() => {
    ...
    const criticas = this.alertas.filter((a) => a.severidad === 'critica' && a.estado !== 'resuelta').length;
    ...
    return [
      { label: 'Activas', valor: activas, borderClass: activas ? 'border-rose-200' : 'border-slate-200', labelClass: 'text-rose-500', valueClass: activas ? 'text-rose-600' : 'text-slate-400' },
      { label: 'Críticas', valor: criticas, borderClass: criticas ? 'border-amber-200' : 'border-slate-200', labelClass: 'text-amber-500', valueClass: criticas ? 'text-amber-600' : 'text-slate-400' },
      { label: 'Asignadas', valor: asignadas, borderClass: 'border-slate-200', labelClass: 'text-slate-400', valueClass: 'text-slate-700' },
      { label: 'Resueltas hoy', valor: resueltas, borderClass: 'border-slate-200', labelClass: 'text-slate-400', valueClass: 'text-slate-700' },
    ];
  });
  ...
  severidadIconClass(s: AlertaSeveridad): string {
    return s === 'critica' ? 'bg-rose-50 text-rose-500' : s === 'advertencia' ? 'bg-amber-50 text-amber-500' : 'bg-blue-50 text-blue-500';
  }
  ...
  severidadBadgeClass(s: AlertaSeveridad): string {
    return s === 'critica' ? 'bg-rose-50 text-rose-600' : s === 'advertencia' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600';
  }

  estadoLabel(e: AlertaEstado): string {
    const map: Record<AlertaEstado, string> = { activa: 'Activa', reconocida: 'Reconocida', asignada: 'Asignada', resuelta: 'Resuelta' };
    return map[e];
  }

  estadoBadgeClass(e: AlertaEstado): string {
    const map: Record<AlertaEstado, string> = { activa: 'bg-rose-50 text-rose-600', reconocida: 'bg-amber-50 text-amber-700', asignada: 'bg-cyan-50 text-cyan-700', resuelta: 'bg-slate-100 text-slate-500' };
    return map[e];
  }

  estadoDotClass(e: AlertaEstado): string {
    const map: Record<AlertaEstado, string> = { activa: 'bg-rose-500', reconocida: 'bg-amber-500', asignada: 'bg-cyan-500', resuelta: 'bg-slate-400' };
    return map[e];
  }
