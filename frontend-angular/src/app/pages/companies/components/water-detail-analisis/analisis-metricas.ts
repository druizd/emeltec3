        ...
        @for (m of metricas; track m.label) {
          <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div class="mt-0.5 flex items-end gap-2">
              ...
              @if (m.delta) {
                <span
                  class="mb-0.5 text-[11px] font-bold"
                  [class]="m.deltaPositivo ? 'text-emerald-600' : 'text-rose-500'"
                >
            ...
            <table class="w-full text-left text-sm">
              <thead>
                <tr class="border-b border-slate-100">
                  ...
                  <th
                    class="pb-2 text-[10px] font-black uppercase tracking-widest text-slate-400 text-right"
                  >
                    Visitas
                  </th>
                  <th
              ...
              <tbody class="divide-y divide-slate-50">
                @for (row of tendencia; track row.mes) {
                  <tr class="group hover:bg-slate-50/60">
                    ...
                    <td class="py-2.5 font-mono text-[12px] text-slate-600 text-right">
                      {{ row.visitas }}
                    </td>
                ...
                <div class="rounded-xl border border-slate-100 bg-slate-50/60 px-3 py-3">
                  <div class="flex items-center justify-between gap-2">
                    <div class="flex items-center gap-2.5">
                      <span
                        class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-cyan-100 text-[12px] font-black text-cyan-700"
                      >
                  ...
                  <div class="mt-2 grid grid-cols-3 gap-2">
                    ...
                    <div class="text-center">
                      <p class="font-mono text-base font-black text-slate-800">
                        {{ tec.horasPromedio }}
                      </p>
                      <p class="text-[9px] font-black uppercase tracking-widest text-slate-400">
                        H promedio
                    ...
                    <div class="text-center">
                      <p
                        ...
                        [class]="resolucionColor(tec.resolucionPrimer)"
                      >
                        {{ tec.resolucionPrimer }}
                      </p>
                      <p class="text-[9px] font-black uppercase tracking-widest text-slate-400">
                        Resolución 1ra
                      </p>
              ...
              @for (sla of slaItems; track sla.label) {
                <div>
                  <div class="flex items-center justify-between mb-1">
                    ...
                    <span
                      class="font-mono text-[12px] font-black"
                      [class]="sla.valor >= sla.meta ? 'text-emerald-600' : 'text-rose-500'"
                      >{{ sla.valor }}%</span
                    >
                  </div>
                  <div class="h-2 w-full overflow-hidden rounded-full bg-slate-100">
                    <div
                      ...
                      [class]="sla.valor >= sla.meta ? 'bg-emerald-400' : 'bg-rose-400'"
                      [style.width]="sla.valor + '%'"
                    ></div>
                  </div>
...
export class AnalisisMetricasComponent {
  ...
  readonly metricas: MetricaServicio[] = [
    ...
    },
    {
      ...
      delta: '0.8 h',
      deltaPositivo: true,
      subtext: 'Promedio ante incidencia',
      icon: 'timer',
    },
    {
      label: 'Resolución 1ra visita',
      valor: '86%',
      delta: '4%',
      deltaPositivo: true,
      subtext: 'Últimos 6 meses',
