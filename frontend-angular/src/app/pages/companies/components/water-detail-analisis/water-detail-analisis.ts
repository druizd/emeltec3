...
type AnalisisSection = 'salud' | 'calendario' | 'reportes' | 'metricas' | 'sugerencias' | 'predictivo';
      ...
      <header class="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
        <div class="flex items-center justify-between gap-3">
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
              <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Análisis</p>
              <h2 class="truncate text-lg font-black leading-tight text-slate-800">Salud, calendario, reportes y métricas</h2>
            </div>
          </div>
          <span class="hidden shrink-0 items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-[11px] font-bold text-emerald-700 sm:inline-flex">
...
export class WaterDetailAnalisisComponent {
  ...
  tabClass(key: AnalisisSection): string {
    ...
    return [
      ...
      active ? 'bg-violet-50 text-violet-700 ring-1 ring-violet-100' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700',
    ].join(' ');
  }
}
