...
@Component({
  ...
  template: `
    <div class="space-y-3">
      <!-- Header informativo -->
      <div
        class="flex items-start gap-3 rounded-2xl border border-cyan-100 bg-cyan-50/60 px-4 py-3"
      >
        <span class="material-symbols-outlined mt-0.5 text-[20px] text-cyan-600 shrink-0"
          >auto_awesome</span
        >
        <div>
          <p class="font-black text-cyan-800 text-sm">
            Sugerencias basadas en el historial del sitio
          </p>
          <p class="text-[12px] text-cyan-700 mt-0.5">
            Estas recomendaciones son generadas automáticamente a partir del comportamiento
            histórico de alertas, mantenciones y lecturas del sitio.
          </p>
        </div>
      </div>
      ...
      <div class="space-y-2">
        @for (s of visibles(); track s.id) {
          <article
            ...
            [class]="prioridadBorde(s.prioridad)"
          >
            <div class="p-4">
              <div class="flex items-start gap-3">
                <!-- Icono categoría -->
                <span
                  [class]="categoriaIconClass(s.categoria)"
      ...
      @if (descartadas().length > 0) {
        <div>
          <button
            type="button"
            (click)="mostrarDescartadas.set(!mostrarDescartadas())"
            class="flex items-center gap-1 text-[12px] font-bold text-slate-400 hover:text-slate-600"
          >
            <span class="material-symbols-outlined text-[16px]">{{
              mostrarDescartadas() ? 'expand_less' : 'expand_more'
            }}</span>
          ...
          @if (mostrarDescartadas()) {
            <div class="mt-2 space-y-1.5">
              @for (s of descartadas(); track s.id) {
                <div
                  class="flex items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5 opacity-60"
                >
                  <p class="text-[12px] font-semibold text-slate-500 line-through">
                    {{ s.titulo }}
                  </p>
                  <button
                    type="button"
                    (click)="restaurar(s)"
                    class="shrink-0 text-[11px] font-bold text-cyan-600 hover:underline"
                  >
                    Restaurar
                  </button>
                </div>
...
export class AnalisisSugerenciasComponent {
  ...
  sugerencias: Sugerencia[] = [
    {
      ...
      descripcion:
        'La batería de respaldo lleva 3 semanas por debajo del 25%. Se recomienda revisión o reemplazo preventivo antes del próximo corte.',
      prioridad: 'alta',
      categoria: 'riesgo',
    ...
    },
    {
      ...
      descripcion:
        'El caudalímetro MAG 5100W tiene su última calibración hace 11 meses. El certificado vence el 14/05/2026.',
      prioridad: 'alta',
    ...
    },
    {
      ...
      descripcion:
        'El umbral actual (2.0 L/s) genera falsas alarmas en horarios nocturnos. Evaluar ajuste a 1.5 L/s fuera de horario operacional.',
      prioridad: 'media',
    ...
    },
    {
      ...
      descripcion:
        'La última revisión de bomba fue el 08/12/2025. El plan considera mantención cada 6 meses.',
      prioridad: 'media',
      categoria: 'mantencion',
    ...
    },
    {
      ...
      descripcion:
        'Se detectaron 12 registros duplicados en el sensor de nivel entre el 02/04 y 03/04, coincidiendo con el gap GPRS.',
      prioridad: 'baja',
  ...
  categoriaIconClass(c: SugerenciaCategoria): string {
    const map: Record<SugerenciaCategoria, string> = {
      mantencion: 'bg-cyan-50 text-cyan-600',
      configuracion: 'bg-violet-50 text-violet-600',
      riesgo: 'bg-rose-50 text-rose-600',
      eficiencia: 'bg-emerald-50 text-emerald-600',
    };
    return map[c];
  }
  ...
  categoriaBadgeClass(c: SugerenciaCategoria): string {
    const map: Record<SugerenciaCategoria, string> = {
      mantencion: 'bg-cyan-50 text-cyan-700',
      configuracion: 'bg-violet-50 text-violet-700',
      riesgo: 'bg-rose-50 text-rose-700',
      eficiencia: 'bg-emerald-50 text-emerald-700',
    };
    return map[c];
  }
