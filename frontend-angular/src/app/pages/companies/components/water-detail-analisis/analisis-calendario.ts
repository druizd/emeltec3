    ...
    <div class="space-y-3">
      ...
      <div class="flex items-center justify-between gap-3">
        <div class="flex items-center gap-2">
          ...
          <span class="min-w-[180px] text-center text-sm font-black text-slate-800">{{
            tituloSemana()
          }}</span>
          <button
            type="button"
          ...
          <button
            type="button"
            (click)="irHoy()"
            class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[12px] font-bold text-slate-600 hover:bg-slate-50"
          >
            Hoy
          </button>
        </div>
        <button
          type="button"
          class="inline-flex items-center gap-1.5 rounded-xl border border-violet-200 bg-violet-50 px-3 py-2 text-[12px] font-bold text-violet-700 hover:bg-violet-100"
        >
          ...
          @for (dia of diasSemana(); track dia.fecha) {
            <div class="px-2 py-3 text-center" [class]="dia.esHoy ? 'bg-cyan-50' : ''">
              ...
              <p
                ...
                [class]="dia.esHoy ? 'text-cyan-700' : 'text-slate-700'"
              >
                {{ dia.numeroDia }}
              </p>
              @if (dia.esHoy) {
          ...
          @for (dia of diasSemana(); track dia.fecha) {
            <div class="p-1.5 space-y-1" [class]="dia.esHoy ? 'bg-cyan-50/40' : ''">
              @for (evento of eventosDelDia(dia.fecha); track evento.id) {
                <div
                  [class]="eventoClass(evento.tipo)"
                  class="rounded-lg px-2 py-1.5 cursor-pointer hover:opacity-90 transition-opacity"
                >
      ...
      <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div class="space-y-2">
          @for (evento of proximosEventos(); track evento.id) {
            <div
              class="flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50/60 px-3 py-2.5"
            >
              <span
                [class]="eventoIconClass(evento.tipo)"
        ...
        <div class="mt-4 flex justify-end">
          <button
            type="button"
            class="inline-flex items-center gap-1 text-[12px] font-bold text-violet-700 hover:underline"
          >
...
export class AnalisisCalendarioComponent {
  ...
  readonly eventos: EventoCalendario[] = [
    {
      ...
      fecha: '2026-05-08',
      hora: '09:00',
      descripcion: 'Revisión preventiva sensores y bomba',
    },
    {
      id: '2',
      ...
      fecha: '2026-05-09',
      hora: '10:30',
      descripcion: 'Revisión operacional con cliente',
    },
    {
      id: '3',
      titulo: 'Vencimiento DGA — May',
      tipo: 'vencimiento_dga',
      fecha: '2026-05-10',
      descripcion: 'Envío reporte mensual a DGA',
    },
    {
      id: '4',
      titulo: 'Cert. calibración vence',
      tipo: 'vencimiento_acreditacion',
      fecha: '2026-05-14',
      descripcion: 'Caudalímetro MAG 5100W',
    },
    {
      id: '5',
      titulo: 'Mantención bomba',
      tipo: 'mantencion',
      fecha: '2026-05-20',
      hora: '08:00',
      descripcion: 'Cambio filtro + calibración caudal',
    },
    {
      id: '6',
      ...
      fecha: '2026-09-15',
      descripcion: 'Renovar antes de agosto',
    },
  ];
  ...
  eventoIcon(tipo: EventoTipo): string {
    const map: Record<EventoTipo, string> = {
      mantencion: 'build',
      vencimiento_dga: 'shield',
      vencimiento_contrato: 'contract',
      vencimiento_acreditacion: 'verified_user',
      reunion: 'groups',
    };
    return map[tipo];
  }
  ...
  eventoIconClass(tipo: EventoTipo): string {
    const map: Record<EventoTipo, string> = {
      ...
      vencimiento_dga: 'bg-rose-50 text-rose-600',
      vencimiento_contrato: 'bg-amber-50 text-amber-600',
      vencimiento_acreditacion: 'bg-slate-100 text-slate-600',
