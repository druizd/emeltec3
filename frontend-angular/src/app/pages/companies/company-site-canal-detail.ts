...
@Component({
  selector: 'app-company-site-canal-detail',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
          ...
          >
            ...
            >
              <button
                ...
                [attr.aria-disabled]="!dgaEnabled"
                [title]="dgaUnavailableMessage"
                id="tab-dga"
                aria-controls="tabpanel-dga"
                [attr.tabindex]="activeDetailTab() === 'dga' && dgaEnabled ? 0 : -1"
              >
                <span class="material-symbols-outlined text-[18px]" aria-hidden="true">layers</span>
                DGA
                @if (!dgaEnabled) {
                  <span
                    class="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400"
                  >
                    Pendiente
                  </span>
                }
...
export class CompanySiteCanalDetailComponent implements OnInit, OnDestroy {
  readonly installationType = 'canal' as const;
  readonly dgaEnabled = false;
  readonly dgaUnavailableMessage = 'Reporte DGA para canal pendiente de configuracion especifica.';
  ...
  activeDetailTab = signal<DetailTab>('operacion');
  ...
  ngOnInit(): void {
    ...
    this.companyService.fetchHierarchy().subscribe({
      next: (res: any) => {
        ...
        if (this.dgaEnabled && this.activeDetailTab() === 'dga') {
          void this.loadDgaReports();
        }
        if (this.dgaEnabled) this.loadUltimoEnvio(siteId);
      },
  ...
  setDetailTab(tab: DetailTab): void {
    if (tab === 'dga' && !this.dgaEnabled) return;
  ...
  cycleDetailTab(delta: 1 | -1 | 0, edge?: 'first' | 'last'): void {
    ...
    const available = all.filter(
      (t) => (t !== 'dga' || this.dgaEnabled) && (t !== 'analisis' || this.isSuperAdmin()),
    );
  ...
  quickActionDisabled(action: { openDga?: boolean; [key: string]: unknown }): boolean {
    return action.openDga === true && !this.currentSiteObraDga();
  }

  /** Tooltip para el botón. Solo informa cuando está deshabilitado. */
  quickActionTitle(action: { openDga?: boolean; [key: string]: unknown }): string {
