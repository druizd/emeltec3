...
@Component({
  ...
  template: `
    <div class="space-y-6 animate-in fade-in duration-300">
      ...
      <!-- Formulario de invitaciÃ³n: solo si NO es readOnly   -->
      <!-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• -->
      @if (!readOnly) {
          ...
          <div class="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">
                JerarquÃ­a Real: {{ companyName() }} / {{ subName() }}
              </p>
            </div>
          </div>
          ...
          <form (submit)="saveUser()" class="space-y-8">
            ...
            <div class="bg-slate-50 p-6 rounded-2xl border border-slate-100 space-y-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5" [class.opacity-40]="newUser.tipo === 'Admin'">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">DivisiÃ³n Asignada</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="space-y-1.5">
                ...
                <input required [(ngModel)]="newUser.apellido" name="apellido" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-primary-container/20 outline-none transition-all text-sm" placeholder="Ej. SÃ¡nchez" />
              </div>
              <div class="space-y-1.5">
              ...
              <div class="space-y-1.5">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">TelÃ©fono *</label>
              ...
              <div class="space-y-1.5 md:col-span-2">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Correo ElectrÃ³nico *</label>
        ...
        <table class="w-full text-left text-sm">
          <thead class="bg-slate-50/50 border-b border-slate-100">
            <tr>
              ...
              @if (!readOnly) {
                <th class="px-8 py-4 font-black text-slate-400 uppercase tracking-widest text-[10px] text-right">AcciÃ³n</th>
              }
            </tr>
          </thead>
...
export class UserManagementComponent implements OnInit, OnChanges {
  ...
  subName = computed(() => {
    ...
    if (!id || tree.length === 0) return 'Buscando DivisiÃ³n...';
  ...
  saveUser() {
    ...
    this.userService.createUser(data).subscribe({
      next: (res: any) => {
        if (res.ok) {
          this.status.set({ type: 'success', msg: 'InvitaciÃ³n enviada.' });
          this.loadUsers();
          this.resetForm();
        }
