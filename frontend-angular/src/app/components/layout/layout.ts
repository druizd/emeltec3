...
@Component({
  ...
  template: `
    ...
    <!-- ── Chatbot flotante ── -->
    @if (chatOpen()) {
      <div class="fixed bottom-[110px] z-50 w-[400px]"
           [style]="'right:' + (chatDocked() ? '-356px' : '24px') + '; transition: right 0.35s cubic-bezier(0.4,0,0.2,1)'">
        ...
        <!-- Panel principal -->
        <div class="flex flex-col overflow-hidden rounded-2xl bg-white"
          ...
          <div class="flex flex-1 flex-col gap-3 overflow-y-auto px-4 py-4"
            ...
            <div class="flex items-start gap-2.5">
              ...
              <div class="max-w-[280px] rounded-2xl rounded-tl-sm bg-white px-3.5 py-2.5 text-[13px] leading-5 text-slate-700"
          ...
          <div class="flex items-center gap-2 border-t border-slate-100 bg-white px-3 py-3">
            <input
              ...
              disabled
            />
            <button class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl transition-colors"
    ...
    <!-- Botón flotante -->
    <div class="group fixed bottom-6 z-50"
         [style]="'right:' + (bubbleHidden() ? '-58px' : '24px') + '; transition: right 0.35s cubic-bezier(0.4,0,0.2,1)'">
